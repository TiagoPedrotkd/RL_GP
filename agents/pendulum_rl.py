import numpy as np
from agents.q_learning import DiscretizedQLearningAgent
from agents.sarsa import DiscretizedSARSAAgent
...
class PendulumRL:
    ...
    def train(self, env, method="qlearning", num_iterations=10000, alpha=0.1, gamma=0.99, epsilon=0.1, policy="epsilon", verbose=True):
        if method.lower() == "qlearning":
            agent = DiscretizedQLearningAgent(self.discretizer, alpha=alpha, gamma=gamma, epsilon=epsilon, policy=policy)
        elif method.lower() == "sarsa":
            agent = DiscretizedSARSAAgent(self.discretizer, alpha=alpha, gamma=gamma, epsilon=epsilon, policy=policy)
        else:
